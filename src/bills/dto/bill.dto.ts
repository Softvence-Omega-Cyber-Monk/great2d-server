...
export class CreateBillDto {
  ...
  @ApiProperty({
    example: 'internet',
    description:
      'Bill category (internet, electricity, water, mobile, gas, tv, streaming, insurance, other, etc.)',
    default: 'other',
  })
  @IsString()
  @IsOptional()
  ...
  @ApiPropertyOptional({
    example:
      'Based on your usage patterns, we recommend negotiating for a 15% discount...',
  })
  @IsString()
  @IsOptional()
...
export class SetSavingsGoalDto {
  @ApiProperty({ example: 500.0, description: 'Monthly savings goal amount' })
  @IsNumber()
  @Min(0)
  goalAmount: number;
}
...
export class PublicUpdateStatusDto {
  ...
  @ApiProperty({
    example: 'negotiating',
    description:
      'New status (draft, sent, negotiating, successful, failed, cancelled)',
  })
  @IsString()
  @IsNotEmpty()
  status: string;
